...
export {
  ...
  isHcloudInstalled,
  isHcloudConfigured,
  getHcloudActiveContext,
  configureOvhcloudCli,
  isOvhcloudInstalled,
  isOvhcloudConfigured,
  getOvhcloudEndpoint,
...
const CLI_TOOLS = [
  ...
  },
  {
    ...
    configure: configurePlaywright,
    getStatus: () => {
      if (!isPlaywrightInstalled()) return chalk.red('not installed');
      if (!isPlaywrightConfigured()) return chalk.yellow('no browsers');
      const version = getPlaywrightVersion();
      return chalk.green(`installed${version ? ` (v${version})` : ''}`);
    },
  },
  {
    id: 'hcloud',
    name: 'Hetzner Cloud CLI (hcloud)',
    configure: configureHcloudCli,
    getStatus: () => {
      if (!isHcloudInstalled()) return chalk.red('not installed');
      if (!isHcloudConfigured()) return chalk.yellow('no context');
      const ctx = getHcloudActiveContext();
      return chalk.green(`configured (${ctx || 'default'})`);
    },
  },
  {
    id: 'ovhcloud',
    name: 'OVHcloud CLI (ovhcloud)',
    configure: configureOvhcloudCli,
    getStatus: () => {
      if (!isOvhcloudInstalled()) return chalk.red('not installed');
      if (!isOvhcloudConfigured()) return chalk.yellow('not configured');
      const endpoint = getOvhcloudEndpoint();
      return chalk.green(`configured (${endpoint || 'ovh-eu'})`);
    },
  },
  {
    id: 'llm',
