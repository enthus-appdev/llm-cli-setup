...
const CLI_TOOLS = [
  {
    id: 'sqlcmd',
    name: 'SQL Environment (sqlcmd + sql-env)',
    configure: async () => {
      await installSqlcmd();
      console.log();
...
export const configureShellEnv = async () => {
  console.log(chalk.gray('[DEBUG] configureShellEnv() called'));
  ...
  if (mode === 'select') {
    ...
    console.log(chalk.gray(`\n[DEBUG] Selected ${toolsToRun.length} tool(s): ${toolsToRun.map((t) => t.id).join(', ')}`));
  }

  console.log(chalk.gray(`[DEBUG] About to run ${toolsToRun.length} tool(s)\n`));
  ...
  for (let i = 0; i < toolsToRun.length; i++) {
    ...
    try {
      ...
      console.error(chalk.red(`\n✗ Error configuring ${tool.name}: ${error.message}`));
      ...
      const { continueSetup } = await inquirer.prompt([
        {
          type: 'confirm',
          name: 'continueSetup',
          message: 'Continue with remaining tools?',
          default: true,
        },
      ]);
