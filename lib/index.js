...
const CLI_TOOLS = [
  ...
  },
  {
    ...
    commands: ['n8nctl workflow list', 'n8nctl workflow pull <id> -r', 'n8nctl variable list'],
  },
  {
    id: 'grafana',
  ...
  },
  {
    ...
    commands: ['m365 spo site list', 'm365 spo file list'],
  },
  {
    id: 'esq',
    ...
    commands: ['esq search documents "query"', 'esq health'],
  },
  {
    id: 'discord',
    ...
    commands: ['discordctl msg list <channel>', 'discordctl msg send <channel> "text"'],
  },
  {
    id: 'llm',
 ...
 * @param {Object.<string, Function>} [options.hooks] - Post-configure hooks keyed by tool id (e.g. { discord: fn, llm: fn })
 */
export const configureShellEnv = async (options = {}) => {
  ...
  for (let i = 0; i < toolsToRun.length; i++) {
    ...
    if (options.hooks?.[tool.id]) {
      await options.hooks[tool.id]();
    }
  }
