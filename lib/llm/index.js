...
Jira uses wiki markup, NOT Markdown. Common mistakes:
- \`### Heading\` doesn't work → use \`h3. Heading\`
- \`**bold**\` doesn't work → use \`*bold*\`
- \`- bullet\` doesn't work → use \`* bullet\`
...
**Where wiki markup works:**
- Issue descriptions ✓
- Issue comments ✗ (plain text only, no formatting)
...
h3. Notes
Any additional context. URLs auto-link: https://example.com
\`\`\`
...
Command-line interface for managing n8n workflows. Perfect for LLM-assisted workflow development.
...
n8n workflow push <dir>                    # Push from manifest (respects dependencies)
...
The \`manifest.json\` tracks workflow relationships and enables pushing in correct dependency order:
...
Typical workflow for LLM-assisted n8n development:

1. \`n8n workflow pull <id> -r -d ./wf\` - Pull workflow tree
2. Read and modify JSON files
3. \`n8n workflow push ./wf\` - Push changes back

Use \`--json\` flag for structured output when needed.`;
...
export const configureLlmTools = async () => {
  ...
  console.log(chalk.gray('This teaches LLMs how to use sqlcmd, sql-env, gh, atl, and n8n.\n'));
  ...
  console.log(chalk.gray('Your AI assistants now know how to use sqlcmd, sql-env, gh, atl, and n8n!'));
