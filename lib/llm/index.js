...
"Flag the ticket" means the impediment flag (\`atl --context prod jira issue flag <key>\`), never a status transition. The NX workflow is one-way with no reject path, so a wrong transition cannot be undone without an admin: never transition to explore, and ask first when a status change seems needed.
