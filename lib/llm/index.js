...
- **Stack login**: \`gcx login <ctx> --oauth\` prints a URL plus a verification code
  ...
  (\`open '<url>'\`), tell the user the verification code so they can match it
...
- **Cloud/GCOM**: prefer a Cloud Access Policy token over \`gcx cloud login\`
  browser OAuth. This is gcx's own recommendation — its help marks the OAuth
  flow EXPERIMENTAL, notes that some grafana.com commands do not work with an
  OAuth-issued token, and that the token cannot be refreshed. The user creates
  the token in the stack UI under Administration → Users and access →
  Cloud access policies, then:
  \`gcx login --context <ctx> --cloud-token glc_...\` (non-interactive).
- **Before fighting any auth or syntax problem**, read the CLI's own bundled
  agent docs: \`gcx agent skills list\`, then
  \`gcx agent skills get setup-gcx -otext\`.
