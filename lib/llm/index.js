...
Official CLI for managing OVHcloud services. Stores API credentials in \`~/.ovh.conf\` (INI, per-endpoint sections like \`[ovh-eu]\`).
...
Credentials live in \`~/.ovh.conf\`. Set the endpoint (default \`ovh-eu\`) in the \`[default]\` section. Override per-command with the \`OVH_*\` env vars.
