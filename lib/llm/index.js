...
\`\`\`bash
export LOKI_ADDR="$(grep 'server:' ~/.grafanactl/config.yaml | awk '{print $2}')/api/datasources/proxy/9"
export LOKI_BEARER_TOKEN="$(grep 'token:' ~/.grafanactl/config.yaml | awk '{print $2}')"

~/bin/logcli labels k8s_deployment_name
~/bin/logcli query '{k8s_deployment_name="<deployment>"}' --limit=20 --since=1h
\`\`\`
...
export const configureLlmTools = async () => {
  ...
  console.log(chalk.gray('This teaches LLMs how to use sqlcmd, gh, atl, n8nctl, and grafanactl.\n'));
  ...
  console.log(chalk.gray('Your AI assistants now know how to use sqlcmd, gh, atl, n8nctl, and grafanactl!'));
