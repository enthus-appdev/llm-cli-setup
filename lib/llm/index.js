...
- **Cloud/GCOM**: do NOT drive \`gcx cloud login\` browser OAuth from an agent
  session — the flow is experimental and its callback handoff is unreliable
  even when the browser-side approval succeeds. Ask the user for a Cloud
  Access Policy token (grafana.com → Security → Access Policies) and run:
  \`gcx login --context <ctx> --cloud-token glc_...\` (non-interactive, reliable).
