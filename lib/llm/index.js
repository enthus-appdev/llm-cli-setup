...
// Per-tool CLI documentation sections
const CLI_TOOL_DOCS = [
  {
    id: 'sqlcmd',
    name: 'SQL Server (sqlcmd)',
    defaultEnabled: true,
    docs: `## SQL Server Access (sqlcmd)
...
- NEVER use \`--confirm\` flag autonomously - it skips safety prompts`,
  },
  {
    ...
    defaultEnabled: true,
    docs: `## Elasticsearch Query CLI (esq)
...
- Configure aliases for frequently used channels to avoid API lookups`,
  },
  {
    ...
    name: 'Playwright',
    defaultEnabled: true,
    docs: `## Playwright (Browser Automation)
...
/**
 * Generate the CLI tools block with markers for selected tools
 */
const generateCliToolsBlock = (selectedIds) => {
  const docs = CLI_TOOL_DOCS.filter((t) => selectedIds.includes(t.id))
    .map((t) => t.docs)
    .join('\n\n');
  return `${BLOCK_START}
# CLI Tools
 ...
 */
const injectToLlmConfig = (tool, selectedIds) => {
  ...
  // Generate the new block with only selected tool docs
  const cliToolsBlock = generateCliToolsBlock(selectedIds);
 ...
 * @param {string[]} [options.selectedDocs] - Tool IDs to include. Defaults to all default-enabled tools.
 */
export const configureLlmTools = async (options = {}) => {
  console.log(chalk.cyan('\n=== LLM Configuration ===\n'));
  console.log(chalk.blue('Configure CLI tools documentation for your AI coding assistants.'));
  console.log(
    chalk.gray(
      'This teaches LLMs how to use the CLI tools available in this development environment.\n'
    )
  );
  ...
  // Use provided selection or default to all default-enabled tools
  const selectedDocs =
    options.selectedDocs ?? CLI_TOOL_DOCS.filter((t) => t.defaultEnabled).map((t) => t.id);
  ...
  for (const tool of GLOBAL_LLM_TOOLS) {
    try {
      const configPath = injectToLlmConfig(tool, selectedDocs);
  ...
  const selectedNames = CLI_TOOL_DOCS.filter((t) => selectedDocs.includes(t.id)).map((t) =>
    t.name.toLowerCase()
  );
  console.log(chalk.green('\n✓ LLM configuration complete'));
  console.log(chalk.gray(`Your AI assistants now know how to use ${selectedNames.join(', ')}!`));
 ...
 */
export { SUPPORTED_TOOLS, detectExistingConfig, BLOCK_START, BLOCK_END, CLI_TOOL_DOCS };
