...
**Read-only mode**: Environments can be configured with \`readonly: true\`. When active, sqlcmd automatically uses \`-K ReadOnly\` (ApplicationIntent=ReadOnly), preventing accidental writes without needing separate database users.
...
# Comments (subcommand pattern)
atl issue comment list PROJ-1234              # List comments
atl issue comment add PROJ-1234 --body "text" # Add comment
atl issue comment edit PROJ-1234 --id <id> --body "new text"
...
**Supported syntax:**
- Standard Markdown: headings, bold, italic, code, lists, links
