...
atl --context prod jira issue flag PROJ-1234 --unflag     # Remove the flag
atl --context prod jira issue flag PROJ-1234 --status     # Check the flag without changing it
...
atl --context prod jira issue comment add PROJ-1234 --reply-to COMMENT_ID --body "Reply"
atl --context prod jira issue comment edit PROJ-1234 --id COMMENT_ID --body "Updated text"
atl --context prod jira issue comment delete PROJ-1234 --id COMMENT_ID
...
**Supported syntax:**
- Standard Markdown: headings, bold, italic, strikethrough, code, lists, links. Use \`[label](url)\`; Jira wiki links such as \`[label|url]\` remain literal text.
- Blockquotes: \`> text\`
...
Jira issue comments are flat rather than threaded. After upgrading to an atl release newer than v1.13.0, \`--reply-to COMMENT_ID\` creates a new comment containing a real mention of the original author and a focused link to the original comment. It does not quote or duplicate the original body:

\`\`\`bash
atl --context prod jira issue comment add PROJ-1234 --reply-to 12345 --body "Thanks, this is verified."
\`\`\`

On atl v1.13.0 and older, do not use \`--reply-to\`: it copied the full original body, including any existing mentions, without mentioning the original author. Write a standalone comment with an explicit \`@[Display Name]\` mention instead.
...
"Flag the ticket" means the impediment flag (\`atl --context prod jira issue flag <key>\`), never a status transition. The NX workflow is one-way with no reject path, so a wrong transition cannot be undone without an admin: never transition a ticket just to explore the workflow, and ask first when a status change seems needed.

Moving a ticket into an active sprint (\`jira issue sprint --sprint-id\`, \`jira sprint move\`) is confirm-first, and so is creating a sub-task under a story that is in the active sprint, because a sub-task inherits the parent's sprint and the change notifies the whole team. File follow-ups and tech debt as standalone tickets with the NX \`Maintenance\` issue type, not as sub-tasks of an active-sprint story.
...
|-------|--------|-------------------|-----|
| Stack API (\`<slug>.grafana.net\`) | dashboards, datasources, log/metric/trace queries, alert rules | \`gcx config check\` fails, 401s | \`gcx login <ctx> --oauth\` |
| Cloud platform API (GCOM, grafana.com) | \`gcx cloud ...\`: stacks, access policies, product mgmt | "context has no cloud auth" | CAP token (below) |

Re-running the stack login never fixes a "context has no cloud auth" error.

**Agent sessions — auth rules:**

- **Stack login**: \`gcx login <ctx> --oauth\` prints a URL plus a verification code
  (agent mode never launches the browser itself). Open that URL exactly once
...
- **Cloud/GCOM**: use a Cloud Access Policy token, persisted via config with
  no login ceremony:
  ...
  one-off command, the \`GRAFANA_CLOUD_TOKEN\` env var is an ephemeral
  override.
- **Realm trap**: a policy created from the stack UI is realm-scoped to that
  stack — its token can never create stacks or org-realm policies, and the
  stack UI does not display realms. Org-wide policies exist only in the org
  portal (grafana.com/orgs/<org>), which needs org-admin access.
- **Before fighting any auth or syntax problem**, read the CLI's own bundled
  agent docs: \`gcx agent skills list\`, then
  \`gcx agent skills get setup-gcx -o text\`.
