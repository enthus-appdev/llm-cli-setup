...
\`gh pr diff\` piped to python3 may produce empty output — save to a temp file first:

\`\`\`bash
gh pr diff {pr} --repo {owner}/{repo} > /tmp/pr.diff 2>&1
python3 -c "
current_file = ''
line_no = 0
...
Apply with \`atl issue edit PROJ-123 --field-file fields.json\` or include in \`atl issue transition\`. Discover allowed values for any select/radio field before guessing:

\`\`\`bash
atl issue field-options --project PROJ --type Bug --field "Severity"
\`\`\`
...
\`atl issue comment add\` against an SD ticket can return success with a comment ID that never actually surfaces in the UI. Always verify with \`atl issue comment list <issue>\` before assuming a post landed.
