...
atl jira issue view PROJ-1234 --json       # View as JSON (custom_fields section)
...
atl jira issue edit PROJ-1234 --description "New description"
atl jira issue edit PROJ-1234 --description "Appended text" --append
atl jira issue edit PROJ-1234 --add-label bug --remove-label wontfix
atl jira issue edit PROJ-1234 --priority High
atl jira issue edit PROJ-1234 --field "Story Points=8"        # Custom field by name
...
atl jira issue transition PROJ-1234 "Done" --field "Resolution=Fixed"  # Transition with required fields

# Issue links
atl jira issue link PROJ-1 PROJ-2 --type Blocks
atl jira issue link PROJ-1 --list                              # List links on an issue
atl jira issue link PROJ-1 --delete <id>                       # Delete a link by ID
atl jira issue link --list-types                               # List available link types

# Web links
atl jira issue weblink PROJ-1234 --url "https://..." --title "Title"

# Sprint management
atl jira issue sprint PROJ-1234 --sprint-id 123
...
atl jira issue comment edit PROJ-1234 --id <id> --body "Updated text"
atl jira issue comment delete PROJ-1234 --id <id>
...
atl jira board rank PROJ-124 PROJ-125 --after PROJ-123 --board-id <board-id>  # Rank relative to another issue
\`\`\`
...
- Use \`atl jira issue attachment PROJ-1234 --list\` to see attachments
...
\`\`\`bash
atl jira issue edit PROJ-1234 --field 'Kontext=Line 1\\n\\nLine 2\\n- Bullet A\\n- Bullet B'
atl jira issue edit PROJ-1234 --field 'Kontext=+++Expand Title\\nHidden content\\n+++'
\`\`\`
...
**Issue security level** has a dedicated flag — don't reach for \`--field-file\`. Set it on create/edit with \`--security "Developer only"\` (name or numeric id); \`--security ""\` on edit clears it. Discover a project's levels with \`atl jira issue field-options --project PROJ --type Bug --field security\`.
...
Apply with \`atl jira issue edit PROJ-123 --field-file fields.json\` or include in \`atl jira issue transition\`. Discover allowed values for any select/radio field before guessing:

\`\`\`bash
atl jira issue field-options --project PROJ --type Bug --field "Severity"
\`\`\`
...
\`atl jira issue comment add\` against an SD ticket can return success with a comment ID that never actually surfaces in the UI. Always verify with \`atl jira issue comment list <issue>\` before assuming a post landed.
...
Transition names vary by Jira instance and language. Use \`atl jira issue transition PROJ-123 --list\` to see available transitions for a specific issue.
