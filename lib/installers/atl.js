...
// Repository for atl-cli - must be set via environment variable
// Example: ATL_CLI_REPO=myorg/atl-cli llm-cli-setup --atl
const ATL_CLI_REPO = process.env.ATL_CLI_REPO || '';
...
const installAtl = async () => {
  // Check if ATL_CLI_REPO is configured
  if (!ATL_CLI_REPO) {
    console.log(chalk.yellow('ATL_CLI_REPO environment variable not set.'));
    console.log(chalk.gray('Set it to your atl-cli repository, e.g.:'));
    console.log(chalk.gray('  ATL_CLI_REPO=myorg/atl-cli llm-cli-setup --atl'));
    return false;
  }
