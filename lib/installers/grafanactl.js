...
const installViaBinary = async () => {
  ...
  try {
    // Get latest version
    const releaseInfo = execSync('curl -s https://api.github.com/repos/grafana/grafanactl/releases/latest', {
      encoding: 'utf8',
    });
    ...
    console.log(chalk.green('✓ grafanactl installed successfully'));
    return { success: true, binaryPath: `${installDir}/grafanactl` };
  } catch (error) {
    console.error(chalk.red(`✗ Failed to install: ${error.message}`));
...
const configureContext = async (binary) => {
  ...
  try {
    // Set server and org
    execSync(`${binary} config set contexts.${contextName}.grafana.server ${serverUrl}`, {
      stdio: 'pipe',
    });
    execSync(`${binary} config set contexts.${contextName}.grafana.org-id ${orgId}`, {
      stdio: 'pipe',
    });
    ...
    if (authMethod === 'token') {
      ...
      if (token) {
        execSync(`${binary} config set contexts.${contextName}.grafana.token ${token}`, {
          stdio: 'pipe',
        });
      }
      ...
      execSync(`${binary} config set contexts.${contextName}.grafana.user ${username}`, {
        stdio: 'pipe',
      });
      execSync(`${binary} config set contexts.${contextName}.grafana.password ${password}`, {
        stdio: 'pipe',
      });
    }

    // Set as current context if it's the first one
    execSync(`${binary} config use-context ${contextName}`, { stdio: 'pipe' });
    ...
    try {
      execSync(`${binary} config check --context ${contextName}`, { stdio: 'inherit' });
...
export const configureGrafanaCli = async () => {
  ...
  if (currentContext) {
    ...
    let contexts = [];
    try {
      const output = execSync(`${binary} config list-contexts`, { encoding: 'utf8', stdio: 'pipe' });
      contexts = output
        .split('\n')
    ...
    if (action === 'add') {
      ...
      if (contexts.length === 0) {
        ...
        try {
          execSync(`${binary} config unset contexts.${contextToRemove}`, { stdio: 'pipe' });
          ...
          if (contextToRemove === currentContext) {
            ...
            if (remaining.length > 0) {
              execSync(`${binary} config use-context ${remaining[0]}`, { stdio: 'pipe' });
    ...
    if (configure) {
      ...
      console.log(chalk.gray(`Configure later with: ${binary} config set contexts.<name>.grafana.server <url>`));
    }
  }
