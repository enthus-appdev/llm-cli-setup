...
const configureContext = async (binary) => {
  ...
  try {
    // Set server and org (use spawnSync to avoid command injection)
    spawnSync(binary, ['config', 'set', `contexts.${contextName}.grafana.server`, serverUrl], {
      stdio: 'pipe',
    });
    spawnSync(binary, ['config', 'set', `contexts.${contextName}.grafana.org-id`, orgId], {
      stdio: 'pipe',
    });
    ...
    if (authMethod === 'token') {
      ...
      if (token) {
        spawnSync(binary, ['config', 'set', `contexts.${contextName}.grafana.token`, token], {
          stdio: 'pipe',
        });
      }
      ...
      spawnSync(binary, ['config', 'set', `contexts.${contextName}.grafana.user`, username], {
        stdio: 'pipe',
      });
      spawnSync(binary, ['config', 'set', `contexts.${contextName}.grafana.password`, password], {
        stdio: 'pipe',
      });
    }

    // Set as current context if it's the first one
    spawnSync(binary, ['config', 'use-context', contextName], { stdio: 'pipe' });
  ...
  if (currentContext) {
    ...
    if (action === 'add') {
      ...
      if (contexts.length === 0) {
        ...
        try {
          spawnSync(binary, ['config', 'unset', `contexts.${contextToRemove}`], { stdio: 'pipe' });
          ...
          if (contextToRemove === currentContext) {
            ...
            if (remaining.length > 0) {
              spawnSync(binary, ['config', 'use-context', remaining[0]], { stdio: 'pipe' });
