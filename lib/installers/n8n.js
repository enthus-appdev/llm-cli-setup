 ...
 */
export const isN8nInstalled = () => commandExists('n8nctl');
...
export const isN8nConfigured = () => {
  try {
    const output = execSync('n8nctl config list --json', { stdio: 'pipe' }).toString();
...
const installN8n = async () => {
  ...
  try {
    ...
    if (!commandExists('n8nctl')) {
      console.error(chalk.red('✗ Install completed but n8nctl command not found'));
...
export const configureN8nCli = async () => {
  ...
  if (runSetup) {
    try {
      execSync('n8nctl config init', { stdio: 'inherit' });
    ...
    console.log(chalk.yellow('\nSkipping (at least one instance required for n8nctl to work).'));
    console.log(chalk.gray('Run: n8nctl config init'));
    return false;
  }
