...
export const installSqlcmd = async () => {
  ...
  if (!pkgManager) {
    console.log(chalk.gray('No supported package manager found. Make sure you have Homebrew installed: https://brew.sh'));
...
for key, env in data.get('environments', {}).items():
    ...
    print(f\\"  {key}\\t- {env.get('name', key)} ({env.get('server', 'no server')}){ro}\\")
