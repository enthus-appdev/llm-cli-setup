...
const getExistingContexts = () => {
  try {
    const output = execSync('sqlcmd config get-contexts 2>/dev/null', { encoding: 'utf8' });
    return output
      .split('\n')
...
/**
 * Add a sqlcmd endpoint
 */
const addEndpoint = (name, address, port) => {
  try {
    execSync(`sqlcmd config add-endpoint --name "${name}" --address "${address}" --port ${port}`, {
      stdio: 'pipe',
    });
    return true;
  } catch {
...
const addUser = (name, username, password) => {
  try {
    execSync(`sqlcmd config add-user --name "${name}" --username "${username}" --password-encryption none`, {
      stdio: 'pipe',
...
/**
 * Add a sqlcmd context
 */
const addContext = (name, endpoint, user) => {
  try {
    execSync(`sqlcmd config add-context --name "${name}" --endpoint "${endpoint}" --user "${user}"`, {
      stdio: 'pipe',
    });
    return true;
  } catch {
  ...
  const { addNew } = await inquirer.prompt([
    {
      ...
      default: existingContexts.length === 0,
    },
  ]);
  ...
  const { addMore } = await inquirer.prompt([
    {
      type: 'confirm',
      name: 'addMore',
      message: 'Add another context?',
      default: false,
    },
  ]);

  if (addMore) {
    return configureSqlEnv();
  }
