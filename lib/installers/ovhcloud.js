 ...
 * pair (OAuth2 client, or legacy application/consumer keys), or a config file
 * that actually carries a credential key (not just an endpoint/empty file).
 */
export const isOvhcloudConfigured = () => {
  ...
  if (oauth2 || legacy) return true;
  try {
    return /^\s*(client_id|application_key)\s*=/m.test(readFileSync(configPath(), 'utf8'));
  } catch {
    return false;
  }
};
