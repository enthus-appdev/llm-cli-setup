...
/**
 * Check if ovhcloud has credentials configured: a config file, or a complete
 * OVH_* env credential pair (OAuth2 client, or legacy application/consumer keys).
 */
...
/**
 * Active endpoint from the config's [default] section (e.g. "ovh-eu"), or null.
 */
export const getOvhcloudEndpoint = () => {
  try {
    ...
    // no readable config — fall through to the env override
  }
  return process.env.OVH_ENDPOINT || null;
};
...
export const configureOvhcloudCli = async () => {
  ...
  if (!isOvhcloudInstalled()) {
    ...
    // install.sh drops the binary in ~/.local/bin, which this process's PATH
    // snapshot won't pick up — don't try to exec it (login) until it resolves.
    if (!isOvhcloudInstalled()) {
      console.log(chalk.yellow('\n⚠ ovhcloud is installed but not yet on this shell PATH.'));
      console.log(
        chalk.gray('Restart your terminal (or add ~/.local/bin to PATH), then run: ovhcloud login')
      );
      return true;
    }
  } else {
  ...
  if (isOvhcloudConfigured()) {
    const source = existsSync(configPath()) ? configPath() : 'via OVH_* environment variables';
