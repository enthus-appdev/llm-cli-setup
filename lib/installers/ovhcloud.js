...
export const isOvhcloudConfigured = () => {
  ...
  try {
    return /^\s*(client_id|application_key)\s*=\s*\S+/m.test(readFileSync(configPath(), 'utf8'));
  } catch {
    return false;
  }
};
...
export const configureOvhcloudCli = async () => {
  ...
  if (isOvhcloudConfigured()) {
    const env = process.env;
    const envCreds =
      (env.OVH_CLIENT_ID && env.OVH_CLIENT_SECRET) ||
      (env.OVH_APPLICATION_KEY && env.OVH_APPLICATION_SECRET && env.OVH_CONSUMER_KEY);
    const source = envCreds ? 'via OVH_* environment variables' : configPath();
