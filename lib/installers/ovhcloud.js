...
/**
 * Check if ovhcloud has credentials configured: a config file, or OVH_* env creds.
 */
export const isOvhcloudConfigured = () =>
  existsSync(configPath()) || Boolean(process.env.OVH_CLIENT_ID || process.env.OVH_APPLICATION_KEY);
...
export const getOvhcloudEndpoint = () => {
  try {
    ...
    // no readable config — fall through
  }
  return null;
};
...
export const configureOvhcloudCli = async () => {
  ...
  if (isOvhcloudConfigured()) {
    console.log(chalk.green(`✓ Credentials configured (${configPath()})`));
