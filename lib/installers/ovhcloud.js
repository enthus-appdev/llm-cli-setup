...
// lines, skips whole-line `;`/`#` comments. Values are kept verbatim after the
// first `=` (matching upstream's Python configparser, which does NOT strip
// inline comments — so a secret containing `#`/`;` is not truncated).
const parseConfig = () => {
  const sections = {};
  let section = null;
  try {
