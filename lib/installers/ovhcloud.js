...
const DEFAULT_ENDPOINT = 'ovh-eu';

// Where install.sh drops the binary (it does not touch PATH).
const INSTALL_BIN_DIR = process.env.XDG_BIN_HOME || path.join(os.homedir(), '.local', 'bin');

// Prepend the install dir to this process's PATH so a freshly-installed ovhcloud
// resolves immediately (detection + login). Called from the install flow only —
// not at module load, so merely importing this file never mutates global PATH.
const ensureInstallBinOnPath = () => {
  if (!(process.env.PATH || '').split(path.delimiter).includes(INSTALL_BIN_DIR)) {
    process.env.PATH = `${INSTALL_BIN_DIR}${path.delimiter}${process.env.PATH || ''}`;
  }
};
...
// Parse ~/.ovh.conf into { section: { key: value } }. Tolerates CRLF and blank
// lines, skips whole-line `;`/`#` comments. The value is everything after the
// first `=` (only leading/trailing whitespace trimmed) — matching upstream's
// Python configparser, which does NOT strip inline comments, so a secret
// containing `#`/`;` is preserved rather than truncated.
const parseConfig = () => {
  const sections = {};
  let section = null;
  try {
    for (const raw of readFileSync(configPath(), 'utf8').split(/\r?\n/)) {
      ...
      const header = line.match(/^\[([^\]]+)\]/); // trailing junk after ] ignored
      if (header) {
...
export const isOvhcloudConfigured = () => {
  ...
  const endpoint = activeEndpoint(sections) || DEFAULT_ENDPOINT;
  // credentials usually live in the endpoint section, but some setups keep them
  // directly in [default] — accept either.
  return sectionHasCredentials(sections[endpoint]) || sectionHasCredentials(sections.default);
};
...
const installOvhcloud = async () => {
  ...
  try {
    if (useBrew) {
      ...
      // Prefer bash for `set -o pipefail` so a failed download (curl -f) propagates
      // instead of sh's exit 0. Fall back to sh (no pipefail) where bash is absent
      // (Alpine/musl) so install still works.
      const hasBash = commandExists('bash');
      const pipeline = `curl -fsSL "${INSTALL_SCRIPT_URL}" | sh`;
      execSync(hasBash ? `set -o pipefail; ${pipeline}` : pipeline, {
        stdio: 'inherit',
        shell: hasBash ? 'bash' : 'sh',
      });
    }
    console.log(chalk.green('✓ OVHcloud CLI installed successfully'));
    if (!useBrew) {
      console.log(
        chalk.gray(`Typically installed to ${INSTALL_BIN_DIR} — ensure it is on your PATH.`)
      );
    }
    return true;
  } catch (error) {
...
export const configureOvhcloudCli = async () => {
  ...
  if (!isOvhcloudInstalled()) {
    ...
    // install.sh writes to INSTALL_BIN_DIR without touching PATH — add it here so
    // the fresh binary resolves for the detection + login below.
    ensureInstallBinOnPath();
    if (!isOvhcloudInstalled()) {
