...
export const ESQ_CLI_REPO = process.env.ESQ_CLI_REPO || 'enthus-appdev/esq-cli';
...
const installEsq = async () => {
  ...
  try {
    const installCmd = `GOPRIVATE=${goprivate} go install github.com/${ESQ_CLI_REPO}/cmd/esq@latest`;
    execSync(installCmd, {
      stdio: 'inherit',
      shell: true,
    });
