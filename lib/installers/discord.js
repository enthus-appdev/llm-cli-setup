...
export const DISCORD_CLI_REPO = process.env.DISCORD_CLI_REPO || 'enthus-appdev/discordctl';
const REPO_DIR = process.env.DISCORD_CLI_DIR || path.join(os.homedir(), 'dev', 'discordctl');
...
export const installDiscordctl = async () => {
  if (!fs.existsSync(REPO_DIR)) {
    console.log(chalk.yellow(`discordctl repository not found at ${REPO_DIR}`));
    console.log(chalk.gray('Clone it first:'));
    console.log(chalk.gray(`  git clone git@github.com:${DISCORD_CLI_REPO}.git ${REPO_DIR}`));
    return false;
  }
  ...
  try {
    ...
    execSync('npm link', { cwd: REPO_DIR, stdio: 'inherit' });
