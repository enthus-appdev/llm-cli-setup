...
export const DISCORD_CLI_REPO = /^[\w.-]+\/[\w.-]+$/.test(rawRepo) ? rawRepo : 'enthus-appdev/discordctl';
...
export const installDiscordctl = async () => {
  ...
  try {
    const result = spawnSync('go', ['install', `github.com/${DISCORD_CLI_REPO}/cmd/discordctl@latest`], {
      stdio: 'inherit',
...
export const configureDiscordCli = async () => {
  ...
  try {
    spawnSync(binary, ['config', 'init'], { stdio: 'inherit' });
