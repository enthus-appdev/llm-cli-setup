...
const installViaBinary = async () => {
  ...
  try {
    ...
    execFileSync('curl', ['-sLf', downloadUrl, '-o', tarballPath]);
    ...
    // Copy (not rename) so it works across filesystems (e.g. /tmp → ~/.local/bin).
  ...
  if (currentContext) {
    ...
    if (action === 'login') {
      ...
      try {
        ...
        console.error(chalk.red(`Failed to view configuration: ${error.message}`));
      }
    }
  } else {
