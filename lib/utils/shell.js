...
/**
 * Add a source line to shell profile if not already present
 * Returns true if line was added, false if already present
 */
export const ensureSourceLine = (profilePath, scriptPath, description = '') => {
  const sourceLine = `[ -f "${scriptPath}" ] && source "${scriptPath}"`;
  const content = readFileSafe(profilePath);

  // Check if already sourced (any form of sourcing this file)
  if (content.includes(scriptPath)) {
    return false;
  }

  // Add the source line with optional comment
...
/**
 * Remove a source line from shell profile
 */
export const removeSourceLine = (profilePath, scriptPath) => {
  const content = readFileSafe(profilePath);
  if (!content.includes(scriptPath)) {
    return false;
  }

  // Remove the source line and any comment above it
